...
## PC3 - Ejecucion final y evidencias

Para revisar el proyecto completo desde cero en Windows PowerShell:
...
Tambien puede generarse todo con:

```powershell
.\scripts\generate_evidence.ps1
```

La evidencia queda organizada en:

- `results/final_run/`: ejecucion final, resumen de limpieza, metricas, modelo y predicciones.
- `results/analysis/`: tablas de analisis exploratorio generadas en Go.
- `results/figures/`: graficos SVG generados en Go.
- `results/benchmarks/`: benchmark comparativo por cantidad de workers.
- `data/processed/`: dataset procesado con features y etiqueta `high_demand`.

Toda la solucion oficial de PC3 se mantiene en Go: procesamiento concurrente, limpieza, feature engineering, entrenamiento ML, evaluacion, analisis, graficos y benchmark.
