...
// PredictRequest contains raw feature values. The API orders and normalizes them
// using the persisted model artifact, preventing clients from changing the model contract.
type PredictRequest struct {
	Features map[string]float64 `json:"features"`
}

// PredictionResult is the response returned by POST /v1/predict.
...
// Predict applies the current API-owned model to raw features. Training may update
// the model concurrently, so this function takes a read snapshot under modelMu and
// releases the lock before the numerical calculation.
